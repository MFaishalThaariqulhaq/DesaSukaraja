...
<div class="container mx-auto px-6 -mt-10 relative z-20 pb-20 bg-pattern min-h-screen">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8" data-aos="fade-up">
    ...
    <div class="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-slate-100" data-aos="fade-right">
    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100" data-aos="fade-left">
      <div class="flex items-center justify-between mb-8">
        <div>
      ...
      <div class="mt-6 text-center bg-emerald-50 rounded-xl p-4">
        <p class="text-xs text-emerald-600 font-semibold uppercase tracking-wide mb-1">Highlight</p>
        <p class="text-sm text-slate-700">Data pendidikan menunjukkan komposisi jenjang pendidikan penduduk</p>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12" data-aos="fade-up">
    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100" data-aos="fade-up">
      <div class="flex items-center justify-between mb-8">
        <div>
    ...
    <!-- Grafik Agama (Pie) -->
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100" data-aos="fade-up" data-aos-delay="100">
      <div class="flex items-center justify-between mb-8">
        <div>
...
<script>
  ...
  @php
  $dusunNames = collect($data) - > pluck('dusun') - > toArray();
  $totalPerDusun = collect($data) - > pluck('total_penduduk') - > toArray();
  $usiaBuckets = [
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_0_5 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_6_11 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_12_17 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_18_25 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_26_35 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_36_45 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_46_60 ?? 0),
    collect($data) - > sum(fn($d) => $d - > kelompok_usia_61_keatas ?? 0),
  ];
  $pendidikan = [
    collect($data) - > sum(fn($d) => $d - > pendidikan_sd ?? 0),
    collect($data) - > sum(fn($d) => $d - > pendidikan_smp ?? 0),
    collect($data) - > sum(fn($d) => $d - > pendidikan_sma ?? 0),
  ];
  $pekerjaan = [
    collect($data) - > sum(fn($d) => $d - > pekerjaan_petani ?? 0),
    collect($data) - > sum(fn($d) => $d - > pekerjaan_karyawan ?? 0),
    collect($data) - > sum(fn($d) => $d - > pekerjaan_wiraswasta ?? 0),
  ];
  $agama = [
    collect($data) - > sum(fn($d) => $d - > agama_islam ?? 0),
    collect($data) - > sum(fn($d) => $d - > agama_kristen ?? 0),
  ];
  @endphp
  ...
  if (ageCtx) {
    new Chart(ageCtx, {
      type: 'bar',
      data: {
        labels: ['0-5', '6-11', '12-17', '18-25', '26-35', '36-45', '46-60', '60+'],
        datasets: [{
          label: 'Jumlah Penduduk',
          data: {
            !!json_encode($usiaBuckets) !!
          },
          backgroundColor: [
  ...
  // 3. Job Chart (Horizontal Bar)
  const jobCtx = document.getElementById('jobChart')?.getContext('2d');
  if (jobCtx) {
    new Chart(jobCtx, {
      type: 'bar',
      data: {
        labels: ['Petani', 'Karyawan', 'Wiraswasta'],
        datasets: [{
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
  ...
  <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100" data-aos="fade-left">
    ...
    <div class="mt-6 text-center bg-emerald-50 rounded-xl p-4">
      <p class="text-xs text-emerald-600 font-semibold uppercase tracking-wide mb-1">Highlight</p>
      <p class="text-sm text-slate-700">Mayoritas penduduk adalah tamatan <span class="font-bold text-slate-900">SMA/Sederajat</span></p>
    </div>
  </div>
</div>
